...
var decrypt = function decrypt(payload, privateKey, passphrase) {
    ...
    try {
        ...
        if (jose.enc !== 'A128CBC-HS256') {
            throw new Error('Unsupported "enc" detected in JOSE. currently only "A128CBC-HS256" supported');
        }
...
var encrypt = function encrypt(value, publicKey, options) {
     ...
     */
    var cipher = crypto_1.createCipheriv(algorithm, cek, iv);
    /**
